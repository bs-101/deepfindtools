...
- 每条资讯都有独立详情页：`/news/{slug}`，用于沉淀可收录内容。
- 公开提交入口：`/submit`，用户提交的工具只进入候选池，不会自动发布。
- 私有后台，可新增、编辑、删除工具和资讯。
- 候选审核池和运营监控：采集脚本只生成待审候选，后台可查看来源健康、最近运行记录和批量审核。
- SEO 基础能力：拆分 sitemap、RSS、`robots.txt`、canonical、Open Graph、JSON-LD。
...
- `/news/[slug]`：SSG + ISR，用于每日资讯的独立详情页。
- `/submit`：SSG + ISR，用于公开提交工具到候选池。
- `/sites/[id]`：SSG + ISR，同时兼容 `/sites/{id}.html`。
- `/sitemap.xml`、`/sitemap-tools.xml`、`/sitemap-news.xml`、`/sitemap-categories.xml`、`/rss.xml` 和 `/robots.txt`：SSR。
...
PRODUCT_HUNT_TOKEN=
LOGO_PREFETCH_LIMIT=260
TOOL_ENRICH_LIMIT=50
```
...
采集器每次运行会写入运行日志，后台“运营监控”模块可查看最近运行、来源健康和候选插入数量。运行日志保存在 Docker volume `deepfindtools_runtime_data`。

常用维护任务：

```bash
docker compose --profile maintenance run --rm logo-prefetch
docker compose --profile maintenance run --rm tool-enrich
```

`logo-prefetch` 会预热远程 Logo 到本地缓存；`tool-enrich` 会为缺少详情内容的工具初始化 Markdown、核心能力、使用场景和 FAQ 初稿。
...
- Standalone news detail pages at `/news/{slug}`.
- Public submission page at `/submit`; submissions go to the review queue and are never published automatically.
- Admin console for adding and editing tools/news.
- Candidate review queue and operations dashboard for source health, recent collector runs, and bulk review.
- Split sitemaps, RSS, robots.txt, canonical tags, Open Graph tags, and JSON-LD for SEO.
...
- `/category/[id]`: SSG + ISR.
- `/daily-ai-news`: SSG + ISR.
- `/news/[slug]`: SSG + ISR for standalone daily news details.
- `/submit`: SSG + ISR for public tool submissions.
- `/sites/[id]`: SSG + ISR, compatible with `/sites/{id}.html`.
- `/sitemap.xml`, `/sitemap-tools.xml`, `/sitemap-news.xml`, `/sitemap-categories.xml`, `/rss.xml`, and `/robots.txt`: SSR.
...
Each collector run writes an operations log. The admin "Operations" dashboard shows recent runs, source health, candidate counts, and logo cache progress. Runtime logs are stored in the Docker volume `deepfindtools_runtime_data`.
...
`logo-prefetch` warms remote logos into the local cache. `tool-enrich` initializes Markdown, feature lists, use cases, and FAQ drafts for tools with thin detail pages.
...
Collector runtime logs are stored in:
...
After deployment, warm the logo cache once:
